...
public class BetterOreRenderer extends BetterBlockRenderer implements ITextureLoadingCallback {
    ...
    public void onRegisterIcons(IconRegister iconRegister) {
        ...
        iconBetterIronOre = ResourceHelper.registerIconCallback(iconRegister, "better_iron_ore", "textures/blocks/", "better_iron_ore", this);
    }
    ...
    public boolean renderWorldBlock(RenderBlocks renderBlocks, IBlockAccess iBlockAccess, Block block, int x, int y, int z) {
        ...
        if (Block.lightOpacity[iBlockAccess.getBlockId(x - 1, y, z)] == 0) {
            tessellator.setBrightness(block.getMixedBrightnessForBlock(iBlockAccess, x - 1, y, z));
            render3DFaceX(icon, x - 0.03125d, y + 0.5d, z + 0.5d, segments, 0.0625d, 1.0f, 1.0f, 1.0f, true, false);
        }
        if (Block.lightOpacity[iBlockAccess.getBlockId(x + 1, y, z)] == 0) {
            tessellator.setBrightness(block.getMixedBrightnessForBlock(iBlockAccess, x + 1, y, z));
            render3DFaceX(icon, x + 1.03125d, y + 0.5d, z + 0.5d, segments, 0.0625d, 1.0f, 1.0f, 1.0f, false, false);
        }
        if (Block.lightOpacity[iBlockAccess.getBlockId(x, y, z - 1)] == 0) {
            tessellator.setBrightness(block.getMixedBrightnessForBlock(iBlockAccess, x, y, z - 1));
            render3DFaceZ(icon, x + 0.5d, y + 0.5d, z - 0.03125d, segments, 0.0625d, 1.0f, 1.0f, 1.0f, true, false);
        }
        if (Block.lightOpacity[iBlockAccess.getBlockId(x, y, z + 1)] == 0) {
            tessellator.setBrightness(block.getMixedBrightnessForBlock(iBlockAccess, x, y, z + 1));
            render3DFaceZ(icon, x + 0.5d, y + 0.5d, z + 1.03125d, segments, 0.0625d, 1.0f, 1.0f, 1.0f, false, false);
            return false;
        }
        return false;
    }
